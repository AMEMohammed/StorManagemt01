...
namespace Out_
{
 ...
 //////// print requst out
        ...
        public DataTable printrequstOutExit(int Check, int UserId, int user)
        {
            ...
            string Query = "SELECT    dbo.RequstOut.IDOut as 'الرقم المخزني' , dbo.Category.NameCategory as 'الاسم' , dbo.TypeQuntity.NameType as 'النوع', dbo.RequstOut.Quntity as 'الكمية', dbo.RequstOut.NameSend as 'اسم المستلم',dbo.Users.Name as 'اسم الموظف' ,dbo.Users.Name as 'العنوان'  ,dbo.PlaceSend.NamePlace as 'الجهة' FROM   dbo.RequstOut INNER JOIN  dbo.Category ON dbo.RequstOut.IDCategory = dbo.Category.IDCategory INNER JOIN    dbo.TypeQuntity ON dbo.RequstOut.IDType = dbo.TypeQuntity.IDType INNER JOIN      dbo.PlaceSend ON dbo.RequstOut.IDPlace = dbo.PlaceSend.IDPlace inner join   Users on Users.IDUSER=@idUser where dbo.RequstOut.Chack=@check and dbo.RequstOut.UserId=@uuu ";
            SqlParameter[] parm = new SqlParameter[3];
            parm[0] =new SqlParameter("@check", Check);
            parm[1]=new SqlParameter("@idUser", UserId);
            parm[2]=new SqlParameter("@uuu", user);
       ...
       public int GetPriceAccount(int iDAccount)
        ...
        ////////// 
        public DataTable SearchINRequsetOutTxtAndDate(int  s, DateTime d1, DateTime d2)
        {




            string Query = "select RequstOut.IDOut as 'رقم الطلب',Category.NameCategory as 'اسم الصنف' ,TypeQuntity.NameType as 'نوع الكمية',PlaceSend.NamePlace as'الجهة المستفيدة' ,RequstOut.Quntity as'الكمية',RequstOut.Price as 'سعر الوحدة',RequstOut.Quntity*RequstOut.Price as'الاجمالي', Currency.NameCurrency as 'العملة',RequstOut.NameOut as'يصرف بامر',RequstOut.NameSend as'باستلام',RequstOut.DateOut as'تاريخ الصرف',Users.Name as 'اسم الموظف' ,RequstOut.DesOut as 'ملاحظات',RequstOut.Chack from Users, Category,TypeQuntity,PlaceSend,RequstOut,Currency where RequstOut.IDCategory=Category.IDCategory and RequstOut.IDType=TypeQuntity.IDType and RequstOut.IDPlace =PlaceSend.IDPlace and RequstOut.UserId=Users.IDUSER and RequstOut.IDCurrency =Currency.IDCurrency  and RequstOut.IDOut =@txt   and DateOut between @d1 and @d2 order by RequstOut.Chack desc ";
            SqlParameter[] parm = new SqlParameter[3];
       ...
       public int GetMAxIDOUt()
        ...
        public int AddNewAccountDetalis(int idcode, int monay, int idsupply, int idout, string Detalis, DateTime d1, int userid,int idcurrn)
        {
            ...
            parm[7] = new SqlParameter("@IDCurrncy", idcurrn);
            return sql.ExcuteQuery(Query, parm);
        }
